...
class AdminOrderController extends AbstractController
{
   ...
   */
  public function index(Request $request, OrderRepository $orderRepo, LineItemRepository $lineItemRepo) {
    $start = $request->query->get('start');
    $end = $request->query->get('end');
    $alreadyPaid = 0;

    if ($this->getUser()->getRole() == "ROLE_LIVREUR") {
  ...
  /**
   * Permet d'afficher les commandes par status
   *
   * @Route("/admin/orders/status", name="admin_order_status")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function status(Request $request, OrderRepository $orderRepo)  {
    $status = $request->query->get('status');
    $orders = $orderRepo->findByStatus($status); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes en attente de paiement
   *
   * @Route("/admin/orders/waitting", name="admin_order_waitting")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function waitting(Request $request, OrderRepository $orderRepo)  {
    $orders = $orderRepo->findOrderNotComplete(); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes par type de paiement
   *
   * @Route("/admin/orders/paymentType", name="admin_order_paymentType")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function paymentType(Request $request, OrderRepository $orderRepo)  {
    $paymentType = $request->query->get('paymentType');
    $orders = $orderRepo->findByPaymentType($paymentType); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes par methode de paiement
   *
   * @Route("/admin/orders/paymentMethod", name="admin_order_paymentMethod")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function paymentMethod(Request $request, OrderRepository $orderRepo)  {
    $paymentMethod = $request->query->get('paymentMethod');
    $orders = $orderRepo->findByPaymentMethod($paymentMethod); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes par expedition
   *
   * @Route("/admin/orders/expedition", name="admin_order_expedition")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function expedition(Request $request, OrderRepository $orderRepo)  {
    $orders = $orderRepo->findByExpedition(); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes par impayées
   *
   * @Route("/admin/orders/impayee", name="admin_order_impayee")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function impayee(Request $request, OrderRepository $orderRepo)  {
    $orders = $orderRepo->findByImpayee(); $total = 0; $alreadyPaid = 0; $remaining = 0;
  ...
  /**
   * Permet d'afficher les commandes à livrer
   *
   * @Route("/admin/orders/livraison", name="admin_order_livraison")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function livraison(Request $request, OrderRepository $orderRepo)  {
    $orders = $orderRepo->findByLivraison(); $total = 0; $alreadyPaid = 0;
  ...
  /**
   * Permet d'afficher les commandes par clients
   *
   * @Route("/admin/orders/customers", name="admin_order_customers")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function customers(Request $request, OrderRepository $orderRepo, LineItemRepository $lineItemRepo)
  {
   ...
   * 
   */
   public function new(ProductRepository $productRepository,VariantRepository $variantRepo, Request $request, ObjectManager $manager, StockListRepository $stockRepo, AdminRepository $adminRepo) {
    $order = new Order();
    ...
    if($form->isSubmitted() && $form->isValid()) {
      $stockList = $request->request->get('stockListId');
   ...
   /**
   * Permet d'éditer une commande
   *
   * @Route("/admin/orders/edit/{id}", name="admin_order_edit")
   * @Security("is_granted('ROLE_ADMIN')")
   * 
   */
   public function edit(Order $order, ProductRepository $productRepository, VariantRepository $variantRepo, Request $request, ObjectManager $manager, TransactionRepository $transactionRepo, StockListRepository $stockRepo, AdminRepository $adminRepo, FolderRepository $folderRepo) {
    $order2 = clone $order;
    $form = $this->createForm(AdminOrderType::class, $order);
    $products = $productRepository->findBy(['archive' => false], ['title' => "ASC"]);
    $variants = $variantRepo->findBy(['archive' => false], ['title' => "ASC"]);
   ...
   /**
   * Permet d'imprimer les commandes
   *
   * @Route("/admin/orders/print", name="admin_order_print")
   * @Security("is_granted('ROLE_ADMIN')")
   * 
   */
   public function print(OrderRepository $orderRepo, Request $request, ObjectManager $manager) {
    $start = $request->query->get('start');
    $end = $request->query->get('end');
   ...
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function upload(Request $request, ObjectManager $manager, OrderRepository $orderRepo, UserRepository $userRepo, FolderRepository $folderRepo)
  {
  ...
  /**
   * Permet de supprimer une ligne de la commande
   *
   * @Route("/admin/lineitem/delete/{id}", name="admin_lineitem_delete")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function deleteLineitem(LineItem $lineItem, StockListRepository $stockRepo, Request $request, ObjectManager $manager)
  {
   ...
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function deleteOrder(Order $order, StockListRepository $stockRepo, ObjectManager $manager, TransactionRepository $transactionRepo, NoteRepository $noteRepo)
  {
  ...
  /**
   * Permet d'exporter une commande
   *
   ...
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function customer(Request $request, OrderRepository $orderRepo)
  {
   ...
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function histories(Request $request, OrderHistoryRepository $orderHistoryRepo, LineItemRepository $lineItemRepo)
  {
  ...
  /**
   * Permet de filtrer l'historique par montant payé
   *
   * @Route("/admin/orders/histories/filter", name="admin_order_filter_histories")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function historiesFilter(Request $request, OrderHistoryRepository $orderHistoryRepo, LineItemRepository $lineItemRepo)
  {
  ...
  /**
   * Permet de filtrer l'historique pour une commande
   *
   * @Route("/admin/orders/history/{id}", name="admin_order_history")
   * @Security("is_granted('ROLE_ADMIN')")
   */
  public function history(Order $order, Request $request, OrderRepository $orderRepo, LineItemRepository $lineItemRepo)
  {
